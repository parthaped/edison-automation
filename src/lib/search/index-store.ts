...
import {
  ...
  type SerializedMiniSearchIndex,
} from "./index-types";
...
function parseJsonl(text: string): SearchIndexRecord[] {
...
async function loadFromBlob(): Promise<LoadedSearchIndex | null> {
  ...
  try {
    ...
    const allRecords = parseJsonl(jsonlText);
...
export class SearchIndexUnavailableError extends Error {
  constructor(
    message = "Search index is not available. Production search falls back to live Omeka results.",
  ) {
    super(message);
