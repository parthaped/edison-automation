#!/usr/bin/env python3
"""Curate v5 training pages without circular kraken_phase2 vision labels."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--train-list", type=Path, default=Path("ml/data/manifests/quality_v5_train_pagexml.txt"))
    parser.add_argument("--frozen-test-list", type=Path, default=Path("ml/data/manifests/frozen_test_52_pagexml.txt"))
    parser.add_argument("--curation-report", type=Path, default=Path("ml/reports/quality_v5_curation.jsonl"))
...
def label_is_trustworthy(row: dict[str, Any]) -> bool:
    """Exclude circular pseudo-labels from kraken_phase2 vision-primary pages."""
    if row.get("vision_provider") == "kraken_phase2" and row.get("label_source") == "vision_primary":
        return False
    if row.get("label_source") == "none":
        return False
    return True
...
def quality_tier(row: dict[str, Any], frozen: set[str]) -> str | None:
    ...
    if row.get("status") != "accepted":
        return None
    if not label_is_trustworthy(row):
        return None
    ...
    if row.get("label_source") != "scripto":
        return None

    if ratio >= 0.78 and mean_cer <= 0.28 and median_cer <= 0.30:
        return "train_tier_a"
    if ratio >= 0.70 and mean_cer <= 0.38 and median_cer <= 0.42:
        return "train_tier_b"
    return None
...
def main() -> int:
    ...
    if not train_paths:
        raise SystemExit("No trustworthy training pages after quality v5 curation.")
    ...
    scripto = sum(1 for r in curated if r.get("label_source") == "scripto")
    gemini = sum(1 for r in curated if r.get("vision_provider") == "gemini")
    print(f"Frozen test pages: {len(set(frozen_paths))}")
    print(
        f"Train pages: {len(set(train_paths))} "
        f"(tier_a={tier_a}, tier_b={tier_b}, scripto={scripto}, gemini={gemini})"
    )
