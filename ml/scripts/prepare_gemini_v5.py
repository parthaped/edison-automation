#!/usr/bin/env python3
"""Curate v5 Gemini fine-tune pages: prefer gemini labels, frozen 52-page test excluded."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--train-list", type=Path, default=Path("ml/data/manifests/gemini_v5_train_pagexml.txt"))
    parser.add_argument("--frozen-test-list", type=Path, default=Path("ml/data/manifests/frozen_test_52_pagexml.txt"))
    parser.add_argument("--curation-report", type=Path, default=Path("ml/reports/gemini_v5_curation.jsonl"))
    parser.add_argument(
        "--allow-non-gemini",
        action="store_true",
        help="Include kraken_phase2/scripto pages if no gemini relabel yet.",
    )
...
def quality_tier(row: dict[str, Any], frozen: set[str], require_gemini: bool) -> str | None:
    ...
    if row.get("split") not in {"train", "validation"}:
        return None
    if require_gemini and row.get("vision_provider") != "gemini":
        return None
    ...
    if ratio >= 0.75 and mean_cer <= 0.32 and median_cer <= 0.35:
        return "train_tier_a"
    if ratio >= 0.68 and mean_cer <= 0.40 and median_cer <= 0.45:
        return "train_tier_b"
    return None
...
def main() -> int:
    ...
    for row in rows:
        ...
        tier = quality_tier(row, frozen, require_gemini=not args.allow_non_gemini)
        if tier is None:
            continue
    ...
    if not train_paths:
        raise SystemExit("No Gemini-labeled training pages after v5 curation.")
    ...
    gemini_count = sum(1 for r in curated if r.get("vision_provider") == "gemini")
    print(f"Frozen test pages: {len(set(frozen_paths))}")
    print(f"Train pages: {len(set(train_paths))} (tier_a={tier_a}, tier_b={tier_b}, gemini={gemini_count})")
