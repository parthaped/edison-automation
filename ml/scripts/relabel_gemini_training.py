#!/usr/bin/env python3
"""Relabel training pages with Gemini via Vercel AI Gateway (break kraken_phase2 circular labels)."""
...
from label_provider import provider_cache_file, resolve_label_provider  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--limit", type=int, default=0, help="Max pages to relabel (0 = all candidates).")
    parser.add_argument("--delay-seconds", type=float, default=2.0)
    parser.add_argument(
        "--checkpoint",
        type=Path,
        default=Path("ml/data/manifests/relabel_gemini_checkpoint.json"),
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional PAGE XML list — only relabel manifest rows matching these paths.",
    )
...
def save_checkpoint(path: Path, completed_ids: set[str], relabeled: int, demoted: int) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        json.dumps(
            {"completed_ids": sorted(completed_ids), "relabeled": relabeled, "demoted": demoted},
            indent=2,
        ),
        encoding="utf-8",
    )
...
def pagexml_allowlist(path: Path | None) -> set[str] | None:
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        if line.strip():
            allowed.add(str(Path(line.strip()).as_posix()))
    return allowed


def is_transient_vision_failure(reason: str) -> bool:
    lowered = reason.lower()
    return any(
        token in lowered
        for token in ("http 429", "http 503", "http 502", "rate_limit", "rate-limit", "temporarily unavailable")
    )


def needs_gemini_relabel(row: dict[str, object], frozen: set[str], allowlist: set[str] | None) -> bool:
    ...
    if allowlist is not None and str(Path(str(pagexml)).as_posix()) not in allowlist:
        return False
    if row.get("vision_provider") == "gemini":
        cache = provider_cache_file(page_id, "gemini", Path("ml/data/cache/vision_labels"))
        if cache.exists():
            return False
    return True
...
def main() -> int:
    ...
    candidates = [
        ...
        if needs_gemini_relabel(row, frozen, allowlist) and str(row.get("id", "")) not in completed
    ]
    if args.limit > 0:
        candidates = candidates[: args.limit]

    if not candidates:
        print("No training pages need Gemini relabel.")
        return 0

    vision_provider = resolve_label_provider("gemini", device=args.device)
    print(f"Gemini relabel: {len(candidates)} pages (provider={vision_provider.name})", flush=True)
    ...
    for index, row in enumerate(candidates, start=1):
        ...
        if not media_id:
            ...
            save_checkpoint(args.checkpoint, completed, relabeled, demoted)
            continue
        ...
        try:
            ...
            if is_transient_vision_failure(message):
        ...
        if result.get("status") != "accepted":
            ...
            if is_transient_vision_failure(reason):
                print(f"  retry later ({reason[:120]})", flush=True)
                time.sleep(args.delay_seconds * 5)
                continue
        ...
        if args.demote_low_quality:
            ...
            if not ok:
                demoted += 1
                row_by_id.pop(page_id, None)
                append_jsonl(args.review_manifest, {**row, **result, "reason": f"gemini_qc:{reason}"})
        ...
        save_checkpoint(args.checkpoint, completed, relabeled, demoted)
        print(
            f"  relabeled ({result.get('validated_lines')} lines, "
            f"provider={result.get('vision_provider')}, source={result.get('label_source')})",
            flush=True,
        )

    print(f"Gemini relabel complete: {relabeled} upgraded, {demoted} demoted", flush=True)
    return 0
